...
export const BackgroundBeamsWithCollision = ({ children, className }) => {
  ...
  return (
    <div
      ref={parentRef}
      className={cn(
        "h-96 md:h-160 bg-black relative flex items-center w-full justify-center overflow-hidden",
    ...
    <div {...props} className={cn("absolute z-50 h-2 w-2", props.className)}>
      ...
      {spans.map((span) => (
        <motion.span
          ...
          className="absolute h-1 w-1 rounded-full bg-linear-to-b from-indigo-500 to-blue-500"
        />
      ))}
    </div>
  );
